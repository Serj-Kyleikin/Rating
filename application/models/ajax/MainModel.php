            ...
            if(isset($_COOKIE['user'])) {
                ...
                if($info == null or $info['user_id'] == NULL) {                  // Не голосовал за этот пост ранее
                    ...
                    if($info['votes_articleID'] == $_POST['article_id']) {       // Обновление рейтинга
                        ...
                        } else return '2:Ошибка сервиса, попробуйте позже!';    // Попытка добавить рейтинг к несуществующему посту
                    }
                ...
                } else return '1:Вы уже проголосовали!';                                      // Повторное голосование

            } else return '0:Проголосовать могут только зарегистрированные пользователи!';    // Неавторизованный пользователь

            return $response;                                                                 // Обновлённые данные рейтинга

        } else return '2:Ошибка сервиса, попробуйте позже!';                                  // Зафиксирована попытка обмана
    }
}